...
This server accepts one (git_basic_auth_policy = HTTP_LDAP), and that is precisely
...
- **A domain password is refused, on purpose.** A server whose
  `git_basic_auth_policy` is `HTTP_LDAP` *will* authenticate one, which is exactly
  why storing one is a bad trade: a Gerrit token is revocable and scoped, a domain
  password is neither.
...
**Label names are never hardcoded.** `Verified`, `Code-Review` and `AI-review` are
one company's `project.config`, not Gerrit's. `gerrit query --submit-records`
...
carry a `tag`; automation tags itself `autogenerated:<kind>` (observed on a live
3.14.1 server: `autogenerated:ai-review`). `--bots` filters on the
...
means something different from one beginning `[suggestion]`. That is a particular
CI job's output format — not Gerrit's, not any bot framework's — so it lives in
the config file and **defaults to empty**. Out of the box the tool prints raw
comments and shows no severity column.
...
2. **A future second binary will `import` core, not spawn the CLI.** Tools like
   `gh-axi` shell out to a vendor CLI only because that binary is foreign (Go).
